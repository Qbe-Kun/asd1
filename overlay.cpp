...
#include <deque>
#include <string>
#include <fstream>   // Upewnij się, że ten nagłówek jest dołączony
#include <iostream>  // Dla std::cerr, opcjonalne
...
private:
	...
	void ShowNotification(int position, const std::string& message, float percentage) {
		...
		ImGui::TextColored(ImColor(GetColorU32(c::accent, alpha)), "%s", "[fsb.cc]");
...
{
	std::string configFolderPath = chaos::utils::appdata_path() + "\\fsb\\configs";
			...
			if (GetBackground  == roblox || GetBackground == hw)
			{
				...
				if (draw)
				{
					...
					ImGui::Begin("IMGUI!", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBringToFrontOnFocus);
					{
						...
						ImGui::TextColored(ImColor(GetColorU32(c::accent)), TEXT("fsb.cc"));
						PopFont();


						SetCursorPos(ImVec2(68, 35));
						ImGui::PushFont(font::lexend_regular);
						ImGui::TextColored(ImColor(GetColorU32(c::accent)), TEXT(".gg/kgKRmJ65t5"));
						PopFont();
						...
						std::vector<std::vector<std::string>> tab_columns = {
											...
											{ "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5","https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5",   },
											{ "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5",  "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5", "https://discord.gg/kgKRmJ65t5", }
						};
								...
								else if (active_tab == 3)
								{
									edited::BeginChild("##Container0", ImVec2((c::background::size.x - 200) / 2, c::background::size.y), NULL);
									{
										...
										for (const auto& file : std::filesystem::directory_iterator(configFolderPath)) {
											...
											if (filePath.extension() == ".cfg" && !std::filesystem::is_directory(file.path())) {
												configFiles.push_back(filePath.string().substr(configFolderPath.length() + 1));
											}
										}

										static std::string currentConfigName = configFiles.empty() ? "" : configFiles[0];
										for (const auto& fileName : configFiles) {
											bool isSelected = (currentConfigName == fileName);
											if (edited::Selectable(fileName.c_str(), isSelected)) {
												currentConfigName = fileName;
												currentConfigName.erase(currentConfigName.find(".cfg"));
												strncpy(configname, currentConfigName.c_str(), IM_ARRAYSIZE(configname));
											}
										}

										InputTextEx("v", "Config Name", configname, 65, ImVec2(GetContentRegionMax().x - style->WindowPadding.x, 40), IM_ARRAYSIZE(configname));

										if (edited::checkboxP("Load", &globals::loadConfigCheckboxState, ImVec2(CalcTextSize("Load").x + 5, 23))) {
											char notificationMessage[100];
											_snprintf_s(notificationMessage, sizeof(notificationMessage), "Successfully Loaded CFG: %s", configname);
											notificationSystem.AddNotification(notificationMessage, 1000);

											chaos::utils::configs::load(configname);
											globals::loadConfigCheckboxState = false;
										}

										ImGui::SameLine();

										if (edited::checkboxP("Save", &globals::saveConfigCheckboxState, ImVec2(CalcTextSize("Save").x + 5, 23))) {
											if (globals::saveConfigCheckboxState) {
												chaos::utils::configs::save(configname);

												if (std::ifstream(configname)) {
													char notificationMessage[100];
													_snprintf_s(notificationMessage, sizeof(notificationMessage), "Successfully Saved CFG: %s", configname);
													notificationSystem.AddNotification(notificationMessage, 1000);
												}
												else {
													std::cerr << "Error: Unable to save configuration file: " << configname << std::endl;
												}

												globals::saveConfigCheckboxState = false;
											}
										}

										ImGui::SameLine();

										if (edited::checkboxP("Delete", &globals::deleteConfigCheckboxState, ImVec2(CalcTextSize("Delete").x + 5, 23))) {
											...
											globals::deleteConfigCheckboxState = false;
										}

										ImGui::SameLine();

										if (edited::checkboxP("Open Folder Location", &globals::openFolderCheckboxState, ImVec2(CalcTextSize("Open Folder Location").x + 5, 23))) {
											notificationSystem.AddNotification("Successfully Opened Config Folder", 1000);
											globals::openFolderCheckboxState = false;
